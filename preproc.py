...
def image_mask_from_summary(df, image_id, resize=False, thick=MASK_THICKNESS):
    # Edited by mrnabati
    mask = np.zeros((ORIGINAL_SIZE, ORIGINAL_SIZE))
...
def prep_image_mask(area_id, is_valtrain=True):
    ...
    with tb.open_file(fn_mask, 'w') as f:
        for image_id in tqdm.tqdm(df.index, total=len(df)):
            im_mask = image_mask_from_summary(df_summary, image_id, resize=True)
...
def get_resized_8chan_image_train(image_id, datapath, bs_mul):
    im = []
...
def train(datapath):
    """
    run the preprocessing for the training stage data
    """
    area_id = directory_name_to_area_id(datapath)
    prefix = area_id_to_prefix(area_id)
    logger.info("Preproc for training on {}".format(prefix))

    # Imagelist (A list of all image IDs)
